...
.diff-view-word-removed-custom .region {
  background-color: ${removedWord};
    ...
    this.subscriptions.add(
      lumine.commands.add("lumine-workspace, .tree-view .selected, .tab.texteditor", {
        "diff-view:enable": {
  ...
  close() {
    ...
    // resolve anything still waiting on marker layers — no diff is coming
  ...
  async diffGit(event, ref = "HEAD") {
    ...
    if (event && event.currentTarget) {
      ...
      // Check if it's a tree-view file item
        ...
        // A tree-view row carries `data-path` itself — the `li` for a file, the
        // `.header` for a directory.
        filePath = target.dataset?.path ?? target.querySelector("[data-path]")?.dataset.path;
      }
      // Check if it's a text editor
      else if (target.classList.contains("editor") || target.tagName === "LUMINE-TEXT-EDITOR") {
    ...
    const projectRepo = await lumine.project.repositoryForPath(filePath);
    if (projectRepo == null) {
      lumine.notifications.addWarning("Diff View", {
  ...
  _getEditorsForGitDiff(filePath, gitHeadText) {
    return lumine.workspace.open(filePath, { split: "left" }).then((editor1) => {
      ...
      const editor2 = lumine.workspace.buildTextEditor({ autoHeight: false });
      ...
      const rightPane = panes[rightPaneIndex] || lumine.workspace.paneForItem(editor1).splitRight();
    ...
    if (this.isEnabled && this.diffView != null) {
      ...
      if (editor1 != null && editor2 != null) {
        ...
        if (pane1 != null && pane2 != null) {
          ...
          const pane1View = lumine.views.getView(pane1);
          const pane2View = lumine.views.getView(pane2);
    ...
    if (!editorsPromise) {
      ...
      if (event && event.currentTarget) {
        ...
        // Check if it's a tree-view file item
          ...
          hasFileTarget = true;
          // A tree-view row carries `data-path` itself — the `li` for a file,
          // the `.header` for a directory.
          params.path = target.dataset?.path ?? target.querySelector("[data-path]")?.dataset.path;
        }
      }
  ...
  updateDiff(editors) {
    ...
    const stdout = (output) => {
      ...
      try {
        ...
        console.error("diff-view could not parse the computed diff", error, output);
        if (this.footerView != null) {
          this.footerView.setNumDifferences(0);
        }
        return;
      }
      this.process.kill();
    ...
    this.process = new BufferedNodeProcess({ command, stdout, stderr, exit });

    // The payload goes over stdin rather than through temp files: writing both
    // buffers out synchronously stalled the window on every change, and one
    // pair of paths in the config directory was shared by every window.
    const payload = JSON.stringify({
      oldText: editors.editor1.getText(),
      newText: editors.editor2.getText(),
      ignoreWhitespace: ignoreWhitespace,
    });
    const child = this.process.process;
    if (child != null && child.stdin != null) {
      child.stdin.on("error", (error) => {
        console.error("diff-view could not send the text to the diff process", error);
      });
  ...
  _getEditorsForQuickDiff() {
    ...
    if (editor2 === null) {
      ...
      if (panes[rightPaneIndex]) {
        ...
        lumine.workspace.paneForItem(editor1).splitRight({ items: [editor2] });
      }
      editor2
        .getBuffer()
        .setLanguageMode(
  ...
  _getEditorsForDiffWithActive(params) {
    ...
    if (activeEditor != null) {
      ...
      if (params.path) {
        ...
        const editor2Promise = lumine.workspace.openURIInPane(filePath, rightPane);
