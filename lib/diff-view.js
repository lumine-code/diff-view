...
module.exports = {
  ...
  activate() {
    ...
    const styleCalculator = new StyleCalculator(lumine.styles, lumine.config);
    this.styleSubscriptions = styleCalculator.startWatching(
      "diff-view-custom-styles",
      ["diff-view.addedColor", "diff-view.removedColor"],
      (config) => {
        // Capture each RGBA string before changing the alpha again: the color
        // object is one instance, so aliasing it would leave every highlight
        // at the word alpha.
        const addedColor = config.get("diff-view.addedColor");
        addedColor.alpha = 0.4;
        const addedLine = addedColor.toRGBAString();
        addedColor.alpha = 0.5;
        const addedWord = addedColor.toRGBAString();
        const removedColor = config.get("diff-view.removedColor");
        removedColor.alpha = 0.4;
        const removedLine = removedColor.toRGBAString();
        removedColor.alpha = 0.5;
        const removedWord = removedColor.toRGBAString();
        return `
.diff-view-added-custom {
  background-color: ${addedLine};
}
.diff-view-removed-custom {
  background-color: ${removedLine};
}
.diff-view-word-added-custom .region {
  background-color: ${addedWord};
}
.diff-view-word-removed-custom .region {
  background-color: ${removedWord};
}`;
      },
    );

    this.subscriptions = new CompositeDisposable();
    this.subscriptions.add(this.styleSubscriptions);
    this.subscriptions.add(
      ...
      lumine.workspace.addWindowSurfaceTransitionObserver(({ item }) => {
        if (!this.isEnabled || this.diffView == null) return;
        const editor1 = this.diffView._editorDiffExtender1?.getEditor();
        const editor2 = this.diffView._editorDiffExtender2?.getEditor();
        if (item !== editor1 && item !== editor2) return;

        // A side-by-side comparison is one surface by definition. Preserve
        // the item the workspace is about to move, then tear the comparison
        // down while both editor DOM trees still share that surface.
        this._preserveEditorOnDisable(item);
        this.disable();
      }),
    );
  ...
  async _prepareTiledSourcePane(editor, missingMessage) {
    const center = lumine.workspace.getCenter();
    let sourcePane = lumine.workspace.paneForItem(editor);
    if (sourcePane == null) throw new Error(missingMessage);

    if (sourcePane.isDetached?.()) {
      await lumine.workspace.attachDetachedPane(sourcePane);
      sourcePane = lumine.workspace.paneForItem(editor);
      if (sourcePane == null) throw new Error(missingMessage);
    }

    if (!center.getTiledPanes().includes(sourcePane)) {
      throw new Error("The editor must be open in the primary workspace center.");
    }

    await lumine.workspace.focusPrimaryWindow();
    return sourcePane;
  },
  ...
  async diffWithSavedFile(event) {
    ...
    let editors;
    try {
      editors = await this._getEditorsForSavedFile(editor1, savedBuffer);
    } catch (error) {
  ...
  async _getEditorsForSavedFile(editor1, savedBuffer) {
    const sourcePane = await this._prepareTiledSourcePane(
      editor1,
      "The current editor is no longer open in a workspace pane.",
    );
    ...
    try {
      ...
      const sourcePaneIndex = panes.indexOf(sourcePane);
      if (sourcePaneIndex < 0) {
        throw new Error("The source editor left the primary workspace center.");
      }
      const rightPaneIndex = sourcePaneIndex + 1;
  ...
  async _getEditorsForGitDiff(filePath, gitHeadText, request, targetEditor = null) {
    ...
    const panesBeforeOpen = new Set(center.getTiledPanes());
    const editor1 =
      targetEditor != null
        ? targetEditor
    ...
    let editor1Pane = lumine.workspace.paneForItem(editor1);
    let ownedPane1 = editor1Pane != null && !panesBeforeOpen.has(editor1Pane) ? editor1Pane : null;
    ...
    try {
      editor1Pane = await this._prepareTiledSourcePane(
        editor1,
        "The file is no longer open in a workspace pane.",
      );
      ownedPane1 = !panesBeforeOpen.has(editor1Pane) ? editor1Pane : null;
    } catch (error) {
  ...
  async _getEditorsForDiffWithActive(params, request) {
    ...
    if (activeEditor != null) {
      ...
      const sourcePane = await this._prepareTiledSourcePane(
        editor1,
        "The active editor is no longer open in a workspace pane.",
      );
      if (!this._isCurrentDiffRequest(request)) return null;
      const panes = lumine.workspace.getCenter().getTiledPanes();
  ...
  _destroyCreatedEditor(editor, ownedPane = null) {
    ...
    if (
      ownedPane != null &&
      lumine.workspace.getCenter().getTiledPanes().includes(ownedPane) &&
