...
module.exports = {
  provideBackgroundTips() {
    return {
      packageName: "diff-view",
      tips: [
        "{% if keys['diff-view:git-head'] %}You can diff the current file against its last commit with {{ 'diff-view:git-head' | keystroke }}{% else %}You can diff two files side by side, or the current file against its last commit.{% endif %}",
        "{% if keys['diff-view:diff-with-saved-file'] %}Compare the current buffer with its saved file using {{ 'diff-view:diff-with-saved-file' | keystroke }}{% else %}Compare the current buffer with its saved file before resolving an external change or deletion.{% endif %}",
      ],
    };
  },
