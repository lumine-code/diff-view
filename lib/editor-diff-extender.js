module.exports = class EditorDiffExtender {
  ...
  clearDynamicViewZones() {
    // One pass: removing each zone through removeViewZone would search and
    // splice the same array once per zone, which is where a heavily wrapped
    // file spends its time.
    var kept = [];
    for (var decoration of this._offsetDecorations) {
      if (decoration.pixelHeight === undefined) {
