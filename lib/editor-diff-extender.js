module.exports = class EditorDiffExtender {
  ...
  syncViewZones(desired) {
    var kept = [];
    var component = this._editor.component;
    ...
    for (var zone of this._viewZones) {
      ...
      if (wantedHeight !== zone.pixelHeight) {
        ...
        // The component only notices a block growing on its own when the
        // element is rendered — its resize observer skips anything not in the
        // DOM, and a zone outside the viewport is not. Invalidating routes the
        // new height through the component's own measure pass, which measures
        // off-screen elements in its measurement area and applies every change
        // under the scroll anchor, so the viewport holds still while the
        // content above it changes size.
        if (component && component.invalidateBlockDecorationDimensions) {
          component.invalidateBlockDecorationDimensions(zone.decoration);
        }
      }
