...
module.exports = class DiffView {
  ...
  _setupSoftWrapListeners(editor1, editor2) {
    ...
    var view1 = lumine.views.getView(editor1);
    var view2 = lumine.views.getView(editor2);
  ...
  _syncViewZoneHeights() {
    ...
    for (var i = 0; i < chunks.length; i++) {
      ...
      if (chunkDiff > 0) {
        ...
        if (position >= 0) {
          this._desiredZones2.set(position, chunkDiff);
        }
      } else if (chunkDiff < 0) {
        ...
        if (position >= 0) {
          this._desiredZones1.set(position, -chunkDiff);
        }
      }
  ...
  _syncUnchangedRegion(start1, end1, start2, end2) {
    ...
    for (var i = 0; i < lineCount; i++) {
      ...
      if (diff > 0) {
        ...
        this._desiredZones2.set(line2, diff);
      } else if (diff < 0) {
        // Line in editor 2 is taller (wraps more), add spacer to editor 1
        this._desiredZones1.set(line1, -diff);
      }
    }
  }
