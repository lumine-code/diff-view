...
function _computeDiffChunks(oldText, newText, isWhitespaceIgnored, timeoutMs) {
  ...
  // Normalize line endings before counting lines, then give both files exactly
  // one trailing newline. This keeps EOL style and trailing-newline state out of
  // the diff while preserving row mappings for files that use bare CRs.
